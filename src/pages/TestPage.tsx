    ...
    <Dialog open={showBonDialog} onOpenChange={(open) => { if (!open) setShowBonDialog(false); }}>
      <DialogContent className="bg-card rounded-[12px]" style={{ borderColor: '#00cc1340' }}>
        ...
        <DialogFooter className="flex gap-3 sm:gap-3">
          <button onClick={() => setShowBonDialog(false)} className="flex-1 py-3 font-extrabold uppercase text-sm rounded-[4px]" style={{ backgroundColor: '#ef4444', color: '#fff', boxShadow: '0 0 12px #ef444480' }}>CANCEL</button>
          <button onClick={() => { setShowBonDialog(false); handleSubmit(); }} className="flex-1 py-3 font-extrabold uppercase text-sm rounded-[4px]" style={{ backgroundColor: '#00cc13', color: '#fff', boxShadow: '0 0 12px #00cc1380' }}>VERWERK</button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
  ...
  const payDialog = (
    <Dialog open={showPayDialog} onOpenChange={(open) => { if (!open) setShowPayDialog(false); }}>
      <DialogContent className="bg-card rounded-[12px]" style={{ borderColor: '#00cc1340' }}>
        ...
        {orderSummary}
        <DialogFooter className="flex gap-3 sm:gap-3">
          <button onClick={() => setShowPayDialog(false)} className="flex-1 py-3 font-extrabold uppercase text-sm rounded-[4px]" style={{ backgroundColor: '#ef4444', color: '#fff', boxShadow: '0 0 12px #ef444480' }}>CANCEL</button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
