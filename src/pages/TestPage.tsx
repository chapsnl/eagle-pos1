  ...
  return (
    <div className="flex-1 flex overflow-hidden h-full relative" style={{ backgroundColor: '#1a1a1a', ...(retourMode ? { border: '4px solid #ef4444', boxShadow: 'inset 0 0 30px rgba(239,68,68,0.15)' } : {}) }}>
      ...
      <div className="flex flex-col h-full" style={{ width: '20%', backgroundColor: retourMode ? '#1a0a0a' : '#121212', borderRight: `1px solid ${retourMode ? '#ef4444' : '#333'}`, transition: 'background-color 0.3s ease' }}>
        ...
        {/* Total at bottom */}
        <div className="px-3 py-3" style={{ backgroundColor: '#00cc13', borderTop: '2px solid #00ff55' }}>
          <div className="flex justify-between items-center font-extrabold" style={{ color: '#fff', fontSize: 'clamp(14px, 3.5vw, 48px)', whiteSpace: 'nowrap', overflow: 'hidden', lineHeight: 1.1 }}>
            <span style={{ flexShrink: 0 }}>TOTAAL</span>
            <span style={{ flexShrink: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis' }}>€{(existingTotal + total).toFixed(2)}</span>
          </div>
        </div>
      </div>
        ...
        {gridLayout.map((row, ri) => (
          <div key={ri} className="flex-1 flex" style={{ minHeight: 0 }}>
            {row.map((cell, ci) => {
              ...
              if (ri === 4 && ci === 0) {
                return (
                  <button key={ci} onClick={() => { setCoatNumber(''); setBagNumber(''); setItems([]); setSessionId(null); setSessionTotal(0); setExistingLogs([]); setPhase('input'); setActiveField(null); setRetourMode(false); }} style={{ flex: cell.span, backgroundColor: '#1a3a6a', color: '#fff' }} className="pos-btn flex items-center justify-center border-[0.5px] border-black/10 p-1 min-w-0 transition-all duration-75"
